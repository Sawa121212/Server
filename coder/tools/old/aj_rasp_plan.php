...
if(isset($_GET['str'])){$str=$_GET['str'];}else $str='0';
...
$sql="select distinct k
 from ".$table_ych_plan."
 where rtrim(ltrim(fac))='".trim($csp)."'
  and fs='".$fs."'
  and course='".$course."'
 order by k";
echo  "<tr><td align='right'>";
...
or id_aud_fond=74 or id_aud_fond=75
...
if($newt=='1'){
  ...
  if($id!='0'){
   ...
   if(trim($potok)!=''){
   if($raud!=''){
      $sql="select * from raspis_baza where id_nedel='".$nedel."' and id_day='".$j."' and id_para='".$i."' and r_k='".$csp."-".$course.$fstr."' and (r_gr='".($r_gr+0)."' and r_pgr='".($r_pgr+0)."')";// защита групп
      $que1=mssql_query($sql,$link);
      $sql="select * from raspis_baza where id_nedel='".$nedel."' and id_day='".$j."' and id_para='".$i."' and id_aud_fond='".$raud."'";// защита аудиторий
      $que2=mssql_query($sql,$link);
      $sql="select * from raspis_baza where id_nedel='".$nedel."' and id_day='".$j."' and id_para='".$i."' and fio1='".$fio1."'";// защита преподавателей
      $que3=mssql_query($sql,$link);
   if(trim($kol)!=''&&$kol!='0'&&$kol!=0){
      $sql="select * from raspis_baza where id_nedel='".$nedel."' and id_day='".$j."' and id_para='".$i."' and r_k='".$r_k1."' and (r_gr='".($r_gr1+0)."' and r_pgr='".($r_pgr1+0)."')";// защита групп
      ...
      if($raud==74||$raud==75) $iq=0; /////////////   Снятие блокировки с аудиторий манеж, спартак
      if($iq==0){
        if($nedel2allsem!=''){
