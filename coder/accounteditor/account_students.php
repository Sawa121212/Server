...
<?php
  echo "<h3 align='center'>Студенты</h3>";
  if ($_SESSION['usertype'] == 1)
  {
  $select= mssql_query("SELECT id,login, email, second_name, first_name, patronymic, usertype FROM users where usertype=4 ORDER BY first_name");

  //Обрабатывает ряд результата запроса
  $num = 1;
  echo "<ul class='collection'>";
    while ($r= mssql_fetch_array($select)) {
    ...
    echo "</ul>";
  //mssql_close($conn_hard);// закрываем подключение к БД.
